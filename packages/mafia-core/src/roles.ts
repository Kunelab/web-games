 ...
 */
/**
 * The line an examiner gets back when its target did nothing worth finding.
 *
 * Exported because two layers need to agree on it: the engine hands it out for
 * an inactive seat, and the bots' reader refuses to treat it as an exoneration.
 */
export const QUIET_TRADE = 'quiet';
...
export const ROLES: Record<RoleId, RoleDef> = {
  ...
  citizen: def({
    ...
    faction: 'town',
    nightAction: null,
    description: 'Aucun pouvoir, une voix. Votre arme est le débat.',
  ...
  sheriff: def({
    ...
    description: 'Chaque nuit, sonde un joueur : suspect ou non.',
    investigated: L.snoop
  }),
  investigator: def({
    id: 'investigator',
    ...
    faction: 'town',
    nightAction: 'examine',
    description: 'Chaque nuit, examine un joueur et découvre son métier probable.',
    investigated: L.snoop
  }),
  detective: def({
    id: 'detective',
    ...
    faction: 'town',
    nightAction: 'track',
    description: 'Chaque nuit, file un joueur et découvre chez qui il est allé.',
    investigated: L.snoop
  }),
  lookout: def({
    id: 'lookout',
    ...
    faction: 'town',
    nightAction: 'watch',
    description: 'Chaque nuit, surveille une maison et voit qui la visite.',
  ...
  coroner: def({
    ...
    faction: 'town',
    nightAction: 'autopsy',
    description: 'Chaque nuit, autopsie un cadavre et découvre son vrai rôle, maquillé ou non.',
  ...
  doctor: def({
    ...
    faction: 'town',
    nightAction: 'heal',
    description: 'Chaque nuit, soigne un joueur et le sauve d’une attaque.',
    investigated: L.healer
  }),
  bodyguard: def({
    id: 'bodyguard',
    name: 'Garde du corps',
    faction: 'town',
    nightAction: 'guard',
    description: 'Protège un joueur ; si on l’attaque, vous tombez avec l’agresseur.',
    investigated: L.rough
  }),
  escort: def({
    id: 'escort',
    name: 'Hôtesse',
    faction: 'town',
    nightAction: 'block',
    description: 'Chaque nuit, retient un joueur chez lui : son action est annulée.',
    investigated: L.night
  }),
  'bus-driver': def({
    id: 'bus-driver',
    ...
    faction: 'town',
    nightAction: 'swap',
    description: 'Chaque nuit, échange deux maisons : tout ce qui visait l’une arrive chez l’autre.',
    investigated: L.wheel
  }),
  vigilante: def({
    id: 'vigilante',
    name: 'Justicier',
    ...
    nightAction: 'kill',
    charges: 3,
    description: 'Trois balles pour rendre justice vous-même. Visez juste.',
  ...
  veteran: def({
    ...
    charges: 3,
    unique: true,
    description: 'En alerte, abat quiconque lui rend visite cette nuit-là.',
    investigated: L.powder
  }),
  jailor: def({
    id: 'jailor',
    ...
    charges: 3,
    unique: true,
    description: 'Le jour, choisit un prisonnier ; la nuit, l’interroge en cellule et peut l’exécuter.',
    investigated: L.keys
  }),
  mayor: def({
    id: 'mayor',
  ...
  mason: def({
    ...
    faction: 'town',
    nightAction: null,
    description: 'Membre de la loge : les frères se connaissent et se parlent la nuit.',
    investigated: L.rough
  }),
  'mason-leader': def({
    id: 'mason-leader',
    ...
    nightAction: 'recruit',
    unique: true,
    description: 'Chaque nuit, peut initier un citoyen à la loge.',
    investigated: L.rough
  }),
  stump: def({
    id: 'stump',
    ...
    nightAction: null,
    nightImmune: true,
    description: 'Un arbre a des racines et un avis. Ne parle pas, ne meurt pas la nuit, vote.',
    investigated: L.dirt
  }),
  ...
  godfather: def({
    ...
    nightAction: 'kill',
    nightImmune: true,
    detectionImmune: true,
    unique: true,
    familyRank: 'leader',
    description: 'Ordonne le meurtre de la nuit. Insoupçonnable, intouchable chez lui.',
    investigated: L.hands
  }),
  mafioso: def({
    id: 'mafioso',
    ...
    nightAction: 'kill',
    familyRank: 'executor',
    description: 'Le bras armé de la famille : exécute l’ordre du Parrain.',
    investigated: L.powder
  }),
  caporegime: def({
    id: 'caporegime',
    ...
    nightAction: 'kill',
    familyRank: 'executor',
    description: 'Lieutenant de la famille. Quand le Parrain tombe, il apprend à sourire comme lui.',
    investigated: L.powder
  }),
  soldato: def({
    id: 'soldato',
    ...
    nightAction: 'kill',
    familyRank: 'executor',
    description: 'Un soldat de plus dans la famille. Les ordres sont les ordres.',
    investigated: L.powder
  }),
  consigliere: def({
    id: 'consigliere',
    ...
    faction: 'mafia',
    nightAction: 'examine',
    description: 'Chaque nuit, découvre le rôle exact d’un joueur.',
    investigated: L.snoop
  }),
  consort: def({
    id: 'consort',
    name: 'Escorte de la famille',
    faction: 'mafia',
    nightAction: 'block',
    description: 'Chaque nuit, retient un joueur chez lui : son action est annulée.',
    investigated: L.night
  ...
  framer: def({
    ...
    faction: 'mafia',
    nightAction: 'frame',
    description: 'Maquille un innocent en suspect pour la nuit.',
    investigated: L.ink
  }),
  blackmailer: def({
    id: 'blackmailer',
    ...
    faction: 'mafia',
    nightAction: 'silence',
    description: 'Chaque nuit, bâillonne un joueur : demain, il votera mais ne dira pas un mot.',
    investigated: L.watcher
  }),
  janitor: def({
    id: 'janitor',
  ...
  agent: def({
    ...
    faction: 'mafia',
    nightAction: 'shadow',
    description: 'Chaque nuit, piste un joueur : qui l’a visité, et chez qui il est allé.',
    investigated: L.watcher
  }),
  beguiler: def({
    id: 'beguiler',
    ...
    faction: 'mafia',
    nightAction: 'hide',
    description: 'Se cache chez quelqu’un : ce qu’on lui destinait frappe son hôte.',
    investigated: L.night
  }),
  disguiser: def({
    id: 'disguiser',
    ...
    faction: 'mafia',
    nightAction: 'imitate',
    description: 'Vole le profil d’un joueur : les enquêteurs verront l’autre visage.',
    investigated: L.paint
  }),
  actress: def({
    id: 'actress',
    name: 'Actrice',
    faction: 'mafia',
    nightAction: 'imitate',
    description: 'Joue le rôle d’un autre : les enquêteurs applaudissent sans comprendre.',
    investigated: L.paint
  }),
  kidnapper: def({
    id: 'kidnapper',
  ...
  heartbreaker: def({
    ...
    faction: 'mafia',
    nightAction: 'charm',
    description: 'Rend un joueur fou d’amour : si votre cœur s’arrête, le sien aussi.',
  ...
  'dragon-head': def({
    ...
    unique: true,
    familyRank: 'leader',
    description: 'Ordonne le meurtre de la nuit pour la Triade. Insoupçonnable, intouchable.',
    investigated: L.hands
  }),
  enforcer: def({
    id: 'enforcer',
  ...
  silencer: def({
    ...
    faction: 'triad',
    nightAction: 'silence',
    description: 'Chaque nuit, bâillonne un joueur : demain, il votera sans un mot.',
    investigated: L.watcher
  }),
  'incense-master': def({
    id: 'incense-master',
    name: 'Maître de l’encens',
    faction: 'triad',
    nightAction: 'clean',
    charges: 3,
    description: 'L’encens efface l’identité du cadavre ; la Triade, elle, apprend tout.',
    investigated: L.chalk
  }),
  informant: def({
    id: 'informant',
    ...
    faction: 'triad',
    nightAction: 'shadow',
    description: 'Chaque nuit, piste un joueur : visites reçues, visites rendues.',
    investigated: L.watcher
  }),
  deceiver: def({
    id: 'deceiver',
  ...
  diva: def({
    ...
    faction: 'triad',
    nightAction: 'imitate',
    description: 'Se pare du visage d’un autre : les enquêteurs n’y verront que du feu.',
    investigated: L.paint
  }),
  ...
  jester: def({
    ...
    faction: 'neutral',
    nightAction: null,
    description: 'Vous gagnez si la ville vous pend. Faites-vous détester.',
    investigated: L.laugh
  }),
  executioner: def({
    id: 'executioner',
    name: 'Bourreau',
  ...
  amnesiac: def({
    ...
    name: 'Amnésique',
    faction: 'neutral',
    nightAction: 'remember',
    description: 'Ne se souvient plus qui il est. Une nuit, au cimetière, ça lui reviendra.',
    investigated: L.quiet
  }),
  scumbag: def({
    id: 'scumbag',
  ...
  auditor: def({
    ...
    nightAction: 'audit',
    charges: 3,
    description: 'Réduit un joueur à néant administratif : son rôle est dissous. Gagne si la ville perd.',
    investigated: L.ink
  }),
  witch: def({
    id: 'witch',
  ...
  cultist: def({
    ...
    nightAction: 'convert',
    suspicious: true,
    description: 'Une nuit sur deux, convertit une âme de la ville. La secte gagne quand elle est majoritaire.',
    investigated: L.herbs
  }),
  'witch-doctor': def({
    id: 'witch-doctor',
    name: 'Guérisseur vaudou',
    faction: 'cult',
    nightAction: 'heal',
    suspicious: true,
    description: 'Le médecin de la secte : soigne les siens comme les autres.',
    investigated: L.healer
  }),
  'serial-killer': def({
    id: 'serial-killer',
