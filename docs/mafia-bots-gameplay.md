...
- **Haste** decides how soon an investigative role speaks. The base patience is 0.15 on days 1 and 2, 0.4 on day 3, 0.7 on day 4, then 1, multiplied by haste; the speak chance is claim rate times patience. A finding nobody is voting on yet goes out, without the badge, on the first day the speak chance (times 1.4 when the seat holds two or more findings) reaches 0.35: day four for an average seat (claim rate 0.7, haste 1), day three for a hasty one (1.4), day five for a slow one (0.6). A hasty seat also confirms a wagon on its suspect from the first vote, a careful one (haste under 1) from the second.
...
2. **A finding that matters now.** If one of its suspects already has votes on it (one for a hasty seat, two for a careful one, see haste in section 2) or is on trial, a Sheriff or Investigator claims its badge and accuses that seat. A finding nobody is looking at is published without the badge on the day the seat's patience runs out (section 2), and the badge stays hidden. This was measured: badging speculative findings cost the town 2.8 points of win rate.
...
- To **accuse** on its own: the top suspect's score must reach (1.7 minus aggression) scaled down by the clock, and its evidence must reach 0.55 (less as the clock runs) and, for a town seat with nothing hard against the target, at least 2.2, moved by up to a tenth either way by aggression (lower for an aggressive seat). Hearsay alone therefore never starts a town wagon; a contradiction, a check or a deduction does.
...
- A **Vigilante** needs two independent instruments or firm evidence of 1.5 before shooting anybody the count did not single out. Firm evidence is hard evidence minus the night reports of voices the record has not settled (no call proven right, no proven role, no town corpse behind it) and minus calls from leaders it has not proven: it is what a family cannot forge with two fresh badges.
...
2. A Lookout whose watched house died names every visitor and puts each on that doorstep. A Detective whose tail ended at the dead house does the same. Both always, unless the seat is of the quietest kind (claim rate under 0.3). A Bus Driver clears the surviving half of a swap where the other half died. An Escort or Jailor, from day three, hints at the seat it held on a night nobody died. A Spy clears the seat the family aimed at and missed. A Doctor announces a save with probability claim rate plus 0.9 times its desperation.
...
4. **Piling on a liar.** Every agenda accuses a contradicted seat while the seats already accusing it number fewer than one plus four times its aggression, rounded: a calm seat speaks while the accusers are few, an aggressive one joins a longer line.
...
8. **What the room is worth to this juror.** If the juror holds nothing hard of its own against the accused, the chorus part of the case counts at its herd: a follower keeps most of it, a sceptic less.
   **Reasonable doubt.** If nothing hard stands against the accused, the doubt is (0.45 minus a quarter of the clock pressure) times (1 minus half the herd) times how thin the case is (1 minus evidence over 3), and the juror votes innocent when the doubt exceeds where its nerve sits in its range (nerve is rolled evenly across seats, so the doubt catches that share of jurors, always the same ones).
9. Evidence under 0.3 with a comfortable clock: innocent.
10. Otherwise the case is evidence plus 0.3 for having been brought to trial, minus the strength of the defence. The bar is 0.7, minus 0.45 times the pressure (the clock, or 0.6 times the bot's own push appetite, whichever is larger), minus 0.4 times (herd minus 0.5). The chance of guilty is a smooth curve around that bar, and the juror votes guilty when that chance exceeds one minus where its suspicion temperament sits in its range: no dice, so a juror answers the same case the same way every time, and jurors differ by temperament. If the case is a toss-up (chance within 0.12 of a half), nothing hard stands, and the clock is comfortable, it abstains. A family seat's cover ballot (rule 3) is this same rule read off the case as the room sees it.
...
- **A proven Veteran's porch is off every list.** If the record has proved seat 4 is the Veteran, no bot heals, kills, watches or visits 4 while any other house is legal. For a town seat the same goes for a seat that claimed Veteran with nobody contesting the claim: a real Veteran needs no visit and a real alert kills the visitor.
...
**Whether to execute.** A Jailor executes when the prisoner's suspicion plus what the cell proved reaches 2.3 minus courage, and something under that number is more than somebody's word: the cell proved something, or the firm evidence reaches 1, or the town has one mistake or none left. The cell proves 1.8 if the prisoner was held on one night the killing stopped, 3.2 if on two such nights, but only 0.6 per quiet night if the board goes quiet on its own half the time when the cell is empty. A Kidnapper or Interrogator executes when the prisoner is worth it to the family: 1.2 for an undisputed badge, 1.2 for a proven town role, half the prisoner's positive trust, plus 1.6 times the clock, against a bar of 1.2 minus 0.4 times courage. On a live table the prisoner's answer in the cell also counts: these rules only apply once the policy has already decided to execute, and they can only spare: a claimed badge nobody disputes, or a plea from a seat the square barely suspects, buys a night with some probability, while a disputed badge or silence buys none.
...
A Consort, Liaison, Blackmailer or Silencer picks the loudest accusers (with slip), away from the knife's house. An Escort picks the most suspicious seat if its suspicion reaches 1, skipping seats its own checks cleared, proven town roles, undisputed badges and seats that have read out a night's work; otherwise the seat the room trusts least.

### Investigative roles

A Sheriff, Investigator, Detective, Consigliere, Administrator, Agent or Informant checks an unchecked seat, preferring the most suspicious, with a full point of random noise added, so early nights are close to random and later nights follow the board. A town examiner skips seats the record has already answered (a proven town role, or an undisputed badge whose calls came true), and once a killer has flipped it checks that killer's closest voting partner first (the seat that voted with it most and never against it). A Lookout watches the revealed Mayor first, then the loudest accusers. A Coroner examines unnamed corpses first. A Spy has no target.
